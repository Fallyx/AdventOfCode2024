...
internal class Day04
{
    ...
    private static readonly List<Vector2> neighborsP2 =
    [
        new Vector2(-1, -1),
        new Vector2(1, -1),
        new Vector2(-1, 1),
    ...
    {
        if (map.TryGetValue(currentPos + neighborsP2[0], out char ul) 
            && map.TryGetValue(currentPos + neighborsP2[1], out char ur)
            && map.TryGetValue(currentPos + neighborsP2[2], out char ll)
            && map.TryGetValue(currentPos + neighborsP2[3], out char lr))
        {
